...
function drawScene(deltaTime) {
  ...
  for ( var i = 0 ; i < 10 ; ++i){
    ...
    let objectWorldPos = [2.5*i, 0.0, 0.0];
    let rotationAxis = [0.0, 1.0, 0.0];
    
    glMatrix.mat4.rotate(modelMatrix, modelMatrix, globalTime+i, rotationAxis);
    glMatrix.mat4.translate(modelMatrix, modelMatrix, objectWorldPos);
    glMatrix.mat4.scale(modelMatrix, modelMatrix, myDrawable.scalingVector);
    ...
    let cameraFocus = [document.getElementById(`lookXVal`).value, document.getElementById(`lookYVal`).value, document.getElementById(`lookZVal`).value-6.0];
...
function initializeMyObject(vertSource, fragSource, objData) {
  ...
  */
  ...
  myDrawable = new Drawable(myShader, bufferMap, null, rawData.vertices.length / 3, rawData.scalingVector);
